...
function kilometerToMeter(kilometer) {
    if (kilometer < 0) {
        return "distance cannot be negative"; //Detecting negative value of kilometer as it is invalid.
    } else {
...
function budgetCalculator(watch, phone, laptop) {
    if (watch < 0 || phone < 0 || laptop < 0) {
        return "Things cannot be negative";//Detecting negative value of the things as it is invalid.
    } else {
        var watch = watch * 50;
        var phone = phone * 100;
...
function hotelCost(day) {
    if (day < 0) {
        return "Day cannot be a negative number";//Detecting negative value as it is invalid.
    } else {
        if (day <= 10) {
...
function megaFriend(name) {
    if (name.length == 0) {
        return "Array cannot be an empty array"// Detecting empty array
    } else {
        var megaName = name[0];
